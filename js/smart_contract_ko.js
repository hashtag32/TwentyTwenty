...
async function createKOContract(
  element,
  typ,
  underlying,
  threshold,
  ...
  txHashContract = await SampleContract.new(
    chairPersonAccount,
    threshold,
    leverage,
    startPrice,
...
function getUnderLyingValue(underlying) {
  return new Promise((resolve, reject) => {
    ...
    var stockValue = php_function_call(
      "getStockValue",
      [underlying],
      "ajaxCallback"
    );
...
function registerContractKO(contractData) {
  php_function_call("AddContractDataKO_creation", [
    contractData.contract_address,
    contractData.typ,
    contractData.underlying,
    contractData.threshold,
    contractData.leverage,
    contractData.pot,
...
async function buyShares(contract_address, amount) {
  ...
  var txHash = contractInstance.buyShare({
    from: firstAccount,
    value: amount,
    gas: gas_estimate,
    gasPrice: gas_price,
  ...
  php_function_call("AddContractDataKO_buyShare", [
    contract_address,
    firstAccount,
    amount,
    ajax_unique.user_id,
